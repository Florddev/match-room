...
import {
  ArrowUpDown,
  Building,
  Check, ChevronDown, Crown, Droplets, Filter,
  Heart, Home, List, MapPin, Phone, PiggyBank, Search, Star, Tag,
  X
} from "lucide-react"
...
export default function HotelsList() {
  ...
  const sortOptions: SortOption[] = [
    ...
    { label: "Plus de chambres", value: "rooms-desc", icon: <Building className="h-4 w-4" /> },
    { label: "Moins de chambres", value: "rooms-asc", icon: <Building className="h-4 w-4" /> },
  ...
  const categories = [
    ...
    { id: "downtown", name: "Centre-ville", icon: <Building className="h-5 w-5" /> },
    { id: "seaside", name: "Bord de mer", icon: <Droplets className="h-5 w-5" /> },
    { id: "mountain", name: "Montagne", icon: <MapPin className="h-5 w-5" /> },
    { id: "countryside", name: "Campagne", icon: <Tag className="h-5 w-5" /> },
    { id: "business", name: "Affaires", icon: <PiggyBank className="h-5 w-5" /> },
  ];
              ...
              >
                ...
                {(minRate > 0 || selectedCities.length > 0 || selectedZipCodes.length > 0 || minRooms > 0) && (
                  <Badge variant="secondary" className="ml-1 bg-blue-100 text-blue-800 hover:bg-blue-200">
                    {(minRate > 0 ? 1 : 0) + selectedCities.length + selectedZipCodes.length + (minRooms > 0 ? 1 : 0)}
                  </Badge>
                )}
              </button>
            </div>
              ...
              <div className="flex space-x-4">
                {categories.map((category) => (
                  <button
                    ...
                    className={`flex items-center gap-3 px-3 py-2 rounded-md font-medium focus:outline-none ${selectedCategories.includes(category.id)
                          ...
                          <div>
                            {uniqueCities
                              ...
                              .map((city) => (
                                <div
                                  key={city}
                                  className={`p-3 hover:bg-gray-100 cursor-pointer ${selectedCities.includes(city) ? "bg-blue-50" : ""
                                    }`}
                          ...
                          <div>
                            {uniqueZipCodes
                              ...
                              .map((zipCode) => (
                                <div
                                  key={zipCode}
                                  className={`p-3 hover:bg-gray-100 cursor-pointer ${selectedZipCodes.includes(zipCode) ? "bg-blue-50" : ""
                                    }`}
      ...
      <div className="container mx-auto px-4 py-6">
        {isLoading ? (
          ...
          <div className="flex flex-col md:flex-row gap-6">
            {/* Liste des hôtels - Visible uniquement si showMap est false */}
                  ...
                  >
                    ...
                    <div className="relative overflow-hidden aspect-[4/3]">
                      ...
                      <button onClick={(e) => toggleFavorite(e, hotel.id)} className="absolute top-3 right-3 z-10">
                        <Heart
                          className={`h-6 w-6 ${favorites.has(hotel.id) ? "fill-red-500 text-red-500" : "text-white stroke-2 drop-shadow-md"
                            }`}
                        />
                      </button>
            ...
            <div className={`${!showMap ? "hidden md:block" : "block"} md:w-2/5 lg:w-1/3 h-[calc(100vh-200px)] sticky top-[100px]`}>
              <div className="h-full rounded-xl overflow-hidden border border-gray-200 shadow-md">
                <LeafletMap filteredHotels={filteredHotels} />
              </div>
              ...
              <div className="md:hidden fixed bottom-6 left-1/2 -translate-x-1/2 z-10">
                ...
                >
                  <List size={20} />
                  <span>Voir {filteredHotels.length} résultats</span>
                </button>
              </div>
            </div>
          </div>
